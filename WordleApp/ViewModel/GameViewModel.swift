...
class GameViewModel: ObservableObject {
    ...
    init() {
        self.board = Array(repeating: Array(repeating: "", count: 5), count: maxAttempts)
        self.rowCompleted = Array(repeating: false, count: maxAttempts)
        self.rowColors = Array(repeating: Array(repeating: .EmpyCellColor, count: 5), count: maxAttempts)
        self.keyColors = Array(repeating: .EmpyCellColor, count: 26)
        self.cellFlipped = Array(repeating: Array(repeating: false, count: 5), count: maxAttempts)
        self.borderColors = Array(repeating: Array(repeating: .gray, count: 5), count: maxAttempts)
    }
    ...
    private func checkGuess() {
        ...
        if guessResult.correctPosition == targetWord.count {
    ...
    private func flipCellInRow(_ row: Int, at index: Int, colors: [Color]) {
        guard row < board.count && index < board[row].count else { return }
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            ...
            if index + 1 < self.board[row].count {
                self.flipCellInRow(row, at: index + 1, colors: colors)
            }
        }
    }
